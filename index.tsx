...
// Fix: Import the Button component which was used but not defined in this file
...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col font-sans selection:bg-orange-100 text-[13px]">
      <div className="bg-slate-900 py-1.5 text-center text-[9px] font-black tracking-widest uppercase text-white shadow-lg relative z-[70]">
        Secure Payments & Global Shipping Available • Use <span className="text-orange-500 underline underline-offset-2">AK-ELITE</span> for 5% Discount
      </div>
      ...
      <main className="flex-grow">
        ...
        <div id="shop-section" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          {filteredProducts.length > 0 ? (
            ...
            <div className="text-center py-32 bg-slate-50 rounded-[48px] border-2 border-dashed border-gray-200">
              <i className="fa-solid fa-cloud-moon text-orange-200 text-5xl mb-6 animate-pulse"></i>
              <h3 className="text-2xl font-black text-slate-900">End of the line</h3>
              <p className="text-slate-400 mt-2 font-bold uppercase text-[10px] tracking-widest">No results found for current filters</p>
              <button onClick={handleReset} className="mt-10 px-10 py-4 bg-slate-900 text-white rounded-full font-black text-[10px] uppercase tracking-widest hover:bg-orange-600 transition-all shadow-xl active:scale-95">Clear Filters</button>
            </div>
          )}
        </div>
        ...
        <section className="bg-slate-950 py-24 text-white">
          <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-16 text-center md:text-left">
            <div className="animate-in fade-in duration-500">
               <div className="flex items-center gap-3 mb-8 justify-center md:justify-start">
                  <div className="w-12 h-12 bg-orange-600 rounded-2xl flex items-center justify-center font-black text-2xl shadow-xl shadow-orange-900/20">Ak</div>
                  <span className="text-3xl font-black tracking-tighter">Ak Shop</span>
               </div>
               <p className="text-slate-400 font-medium leading-relaxed mb-10 text-sm">Providing exceptional curated items for discerning customers worldwide. Quality isn't a goal; it's our minimum standard.</p>
               <div className="flex gap-4 justify-center md:justify-start">
                  <a href="#" className="w-11 h-11 bg-slate-900 rounded-2xl flex items-center justify-center hover:bg-orange-600 transition-all border border-white/5"><i className="fa-brands fa-facebook-f"></i></a>
                  <a href="#" className="w-11 h-11 bg-slate-900 rounded-2xl flex items-center justify-center hover:bg-orange-600 transition-all border border-white/5"><i className="fa-brands fa-instagram"></i></a>
                  <a href="#" className="w-11 h-11 bg-slate-900 rounded-2xl flex items-center justify-center hover:bg-orange-600 transition-all border border-white/5"><i className="fa-brands fa-linkedin-in"></i></a>
               </div>
            </div>
            <div>
            ...
            <div>
               <h4 className="text-[11px] font-black uppercase tracking-[0.3em] text-orange-500 mb-10">Direct Support</h4>
               <p className="text-slate-400 text-sm font-medium mb-8">Our support team is active 24/7 to handle your elite requests.</p>
               <div className="space-y-4">
                  <a href={`https://wa.me/${settings.whatsapp}`} target="_blank" className="flex items-center gap-4 text-sm font-black hover:text-orange-500 transition-colors group">
                  ...
                  <div className="flex items-center gap-4 text-sm font-black group">
          ...
          <div className="max-w-7xl mx-auto px-6 mt-24 pt-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6">
             <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest">© 2024 Ak Shop Elite. Handcrafted for excellence.</p>
      ...
      {isOrderHistoryOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
           ...
           <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-2xl p-8 md:p-10 relative animate-in zoom-in-95 max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-8 border-b pb-6">
                 <h2 className="text-2xl font-black text-slate-900 flex items-center gap-3">
                    <i className="fa-solid fa-box-open text-orange-600"></i> Purchasing History
                 </h2>
                 <button onClick={() => setIsOrderHistoryOpen(false)} className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center hover:bg-orange-600 hover:text-white transition-all shadow-sm">
              ...
              {!user ? (
                <div className="text-center py-16">
                   <p className="text-slate-400 font-black uppercase text-[11px] tracking-widest mb-6">Security Access Required</p>
                   <Button size="lg" className="rounded-2xl" onClick={() => { setIsOrderHistoryOpen(false); setIsAuthModalOpen(true); }}>Sign In to View</Button>
                </div>
              ) : userOrders.length === 0 ? (
                <div className="text-center py-16">
                   <div className="w-20 h-20 bg-slate-50 rounded-[32px] flex items-center justify-center text-slate-200 text-3xl mx-auto mb-6">
                      <i className="fa-solid fa-receipt"></i>
                   </div>
                   <p className="text-slate-400 font-black uppercase text-[11px] tracking-[0.3em]">No orders recorded</p>
                </div>
              ) : (
                <div className="space-y-6">
                   {userOrders.map(order => (
                     <div key={order.id} className="bg-slate-50 rounded-3xl p-6 border border-slate-100 hover:shadow-lg hover:bg-white transition-all">
                        <div className="flex justify-between items-start mb-6">
                           <div>
                              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{order.date}</p>
                              <p className="text-base font-black text-slate-900 mt-1">{order.id}</p>
                           </div>
                           <span className="bg-orange-600 text-white text-[9px] font-black uppercase px-4 py-1.5 rounded-full shadow-xl shadow-orange-900/10">{order.status}</span>
                        </div>
                        <div className="space-y-3 border-t border-slate-200/50 pt-5">
                           {order.items.map((item, idx) => (
                             <div key={idx} className="flex justify-between text-[12px] font-bold text-slate-600">
                                <div className="flex flex-col">
                                  <span>{item.quantity}x {item.name}</span>
                                  {item.selectedOptions && Object.entries(item.selectedOptions).map(([k, v]) => (
                                    <span key={k} className="text-[10px] text-slate-400 font-black uppercase">{k}: {v}</span>
                                  ))}
                                </div>
